...
app = Flask(__name__)
pfad = ""
simulate_switcher_error = 0         # damit können wir Fehler des Switchers simulieren (keine Antwort und so)
info_fuer_webserver = []
...
def argu():
    ...
    global debug
    global simulate_switcher_error
    ...
    parser.add_argument("-f", help="Simulate Switcher Fehler", default=0, type=int)
    ...
    simulate_switcher_error = args.f
...
def togggle_zuhause():
    ...
    if  (retcode[1].find('ackn') != -1):        # switcher sendet ackn, very good
        ...
        retco.append(8)                         # switcher sendet was anderes als  ackn
    return(retco)     
...
#    print (message) 
    ...
    if simulate_switcher_error == 2 :                   # wert 1 heisst: statusabfrage und wetter ohne Antwort von switcher
        mypri.myprint (DEBUG_LEVEL1, "swserver2.py simuliert KEINE ANTWORT vom switcher")  
        retco.append(9)
        retco.append(keine_antwort_simuliert)
        del ipc_instance
        return(retco)       
...
def get_status(status_art):
    ...
    if simulate_switcher_error == 1 :                   # wert 1 heisst: statusabfrage und wetter ohne Antwort von switcher
        mypri.myprint (DEBUG_LEVEL1, "swserver2.py simuliert KEINE ANTWORT vom switcher")  
        retco.append(9)
        retco.append(keine_antwort_simuliert)
        del ipc_instance
...
if __name__=='__main__':
    ...
    options = argu()        # cammandline args holen    
    sleep(2)
